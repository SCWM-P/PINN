...
class PhysicsInformedNN:
    def __init__(self, layers, connections, device, T, xEvent, yEvent, T0, xEvent0, yEvent0, validation_ratio=0.2):
        """
    ...
    def Denormalization(self, xEvent, T, yEvent):
        ...
        return xEvent, T, yEvent, self.y0
    ...
    def PIloss(self, xEvent, T, yEvent):
        ...
        定义基于物理方程的残差函数
        """
        xEvent, T, yEvent, _ = self.Denormalization(xEvent, T, yEvent)
    ...
    def y0Loss(self, xEvent, T, yEvent):
        """
        定义y0的损失函数
        """
        _, _, yEvent = self.rotate(xEvent, T, yEvent, self.gamma)
        y0_loss = torch.nn.functional.mse_loss(yEvent, torch.ones_like(yEvent)*self.y0)
        return y0_loss
    ...
    def train(self, epochs):
        ...
        for epoch in range(epochs):
            ...
            loss_y0 =  self.y0Loss(self.x_train, self.T_train, self.y_train)

            loss = loss_y + 10*loss_PI + 10000*loss_rotate + 100*loss_y0  # 这里可以通过调节系数来调整惩罚权重
            ...
            if epoch % 10 == 0:
                ...
                print(f'=======Epoch {epoch}=======\n'
                        ...
                        f'a:{self.a.item():.3f},a_grad:{self.a.grad.item():.5f}\n'
                        f'b:{self.b.item():.3f},b_grad:{self.b.grad.item():.5f}\n'
                        f'omega:{self.omega.item():.3f},omega_grad:{self.omega.grad.item():.5f}\n'
                        f'zeta:{self.zeta.item():.3f},zeta_grad:{self.zeta.grad.item():.5f}\n'
...
# Configurations
layers = [2, 150, 150, 150, 150, 150, 150, 150, 150, 1]
connections = [0, 1, 2, 3, 4, 5, 6, 5, 4, 2]
print('=======Data Loading Done!=======')
print('======Model Initialization=====')
pinn = PhysicsInformedNN(layers, connections, device, T, xEvent, yEvent, T0, xEvent0, yEvent0)
...
# 绘制验证集的三维散点图
x_val, T_val, y_val, y0_val = pinn.Denormalization(pinn.x_val, pinn.T_val, pinn.y_val)
x_val_R, T_val_R, y_val_R = pinn.rotate(x_val, T_val, y_val, pinn.gamma)
...
y_pred_val = y_pred_val * pinn.yEvent_std + pinn.yEvent_mean
...
ax.scatter(x_val, T_val, y_pred_val, color='r', label='Predicted (CNN)')
ax.set_xlabel('X')
ax.set_ylabel('T (s)')
ax.set_zlabel('Y')
ax.legend()
